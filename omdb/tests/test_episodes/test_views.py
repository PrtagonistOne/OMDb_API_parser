...
def test_filter_episode(crud_APIFactory):
    ...
    filter_by_rating_title = Episode.objects.filter(imdb_rating__gt=8.8).filter(
        title__icontains="Is"
    )
    ...
    assert len(response_rating.data["results"]) == len(filter_by_imdb_rating)
    assert len(response_rating_title.data["results"]) == len(filter_by_rating_title)
    assert len(response_released.data["results"]) == len(filter_by_release_date)
